...
const parse$1 = (source) => parse(source, {
	sourceType: 'module',
	ecmaVersion: 11,
	locations: true
});
...
class Parser {
	...
	read_destructure_pattern() {
		...
		const code = full_char_code_at(this.template, i);
		if (acorn.isIdentifierStart(code, true)) {
		...
		return (parse_expression_at(`(${pattern_string} = 1)`, 0) ).left ;
	}
...
function traverse_destructure_pattern(
	node,
	callback
) {
			...
			case "ObjectPattern":
				for (let i = 0; i < node.properties.length; i++) {
					...
					if (property.type === "Property") {
						...
						traverse((property ) , node.properties, i);
					}
				}
				break;
			case "RestElement":
...
class AwaitBlockBranch extends Wrapper {
	...
	render_destructure(block, value, node, index) {
 ...
 = [];}
	...
	generate(result) {
		...
		if (result) {
			...
			const banner = `${this.file ? `${this.file} ` : ``}generated by Svelte v${'3.22.0'}`;
